...
// Get Libelle Articles if subscription to TGF is true
if ($LBL == 1) {
...
WHEN EXISTS (
  ...
  ELSE FALSE END) AS liked FROM article WHERE datum BETWEEN subdate(CURDATE(),1) AND CURDATE() AND verborgen = 0 AND site = 'Libelle'";
...
WHEN EXISTS (
  ...
  ELSE FALSE END) AS liked FROM article WHERE datum BETWEEN subdate(CURDATE(),1) AND CURDATE() AND verborgen = 0 AND site = 'Beautylab'";
